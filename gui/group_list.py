...
class GroupItemWidget(QWidget):
    ...
    def init_ui(self, group_item:wg.G_Item):
        ...
        self.group_item_name_label = QLabel(group_item.g_i_name)
        group_item_name_font=self.group_item_name_label.font()
        group_item_name_font.setWeight(QFont.Bold)
        ...
        self.group_item_description_label = QLabel(group_item.g_i_description)
        self.group_item_description_label.setAlignment(Qt.AlignmentFlag.AlignLeft)
        self.group_item_count_label = QLabel(f"数量: {group_item.g_i_count}")
        self.group_item_price_label = QLabel(f"{group_item.g_i_price}元")
...
class GroupInfoWidget(QWidget):
    ...
    def init_ui(self, group:wg.Group):
        ...
        self.group_end_time_label = QLabel(f"结束时间: {group.g_end_time}")
        ...
        vbox = QVBoxLayout()
        vbox.addLayout(hbox)
        vbox.addLayout(self.group_description_label)
...
class GroupListWidget(QWidget):
    ...
    def __init__(self,customer:wg.Customer):
        super().__init__()
        self.init_ui(customer)
    
    def init_ui(self,customer:wg.Customer):
    ...
    def join_group(self, group:wg.Group):
        asyncio.run(services.join_group_with_param(self.customer.c_phone, group.g_id))
